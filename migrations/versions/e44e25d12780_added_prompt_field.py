"""Added prompt field

Revision ID: e44e25d12780
Revises: 
Create Date: 2025-06-27 23:14:58.365591

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = 'e44e25d12780'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('prompts')
    with op.batch_alter_table('prompts_history', schema=None) as batch_op:
        batch_op.add_column(sa.Column('prompt', sa.Text(), nullable=False))
        batch_op.add_column(sa.Column('answer', sa.Text(), nullable=False))
        batch_op.drop_column('prompt_text')
        batch_op.drop_column('ai_response')

    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('prompts_history', schema=None) as batch_op:
        batch_op.add_column(sa.Column('ai_response', sa.TEXT(), nullable=False))
        batch_op.add_column(sa.Column('prompt_text', sa.TEXT(), nullable=False))
        batch_op.drop_column('answer')
        batch_op.drop_column('prompt')

    op.create_table('prompts',
    sa.Column('id', sa.INTEGER(), nullable=False),
    sa.Column('text', sa.TEXT(), nullable=False),
    sa.Column('category', sa.VARCHAR(length=100), nullable=True),
    sa.Column('user_id', sa.INTEGER(), nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    # ### end Alembic commands ###
